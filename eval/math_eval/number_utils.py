...
def compare_two_numbers(p, gt):
    try:
        ...
        if math.isnan(p):
            print(f'** compare_two_numbers p in nan {p} {gt}')
            return False
        if isinstance(gt, int):
            print(f'** compare_two_numbers int {p} {gt}')
            return round(p) == gt
        else:
            print(f'** compare_two_numbers float {p} {gt}')
            return within_eps(pred=p, gt=gt)
    except Exception:
        print('** compare_two_numbers Exception')
        return False
...
def compare_two_list(pred, gt):
    if not isinstance(pred, list):
        ...
        print('** pred list length wrong')
        return False
    elif any([not isinstance(x, (int, float)) for x in pred]):
        print('** pred lists not number')
        return False
    else:
        pred = sorted(pred)
        gt = sorted(gt)
